...
from transform_perspective import TransformPerspective
...
def main():
    ...
    tp = TransformPerspective() # change shape if not (1280,720)
    ...
    try:
        while user_stream.wait_key() is True:
            ...
            if cfg['transform_perspective']:
                inp_frame = tp.transform_image(inp_frame)
            ...
            if cfg['pers_filter_needed']:
                # remove boxes which are not fitting into limits for w,h
                pers_boxes = [pb for pb in pers_boxes if pb.w_h_is_in_pers_range()]
