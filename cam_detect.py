...
from cam_boxes import BGR_WHITE,BGR_GREEN,BGR_RED
...
def main():
    ...
    if cfg['save_output_frames'] is True:
        outp_stream = OutputStream(cfg['out_file_name'],inp_stream.frame_shape,cfg['out_file_fps'],mode='replace_old')
    if cfg['save_boxed_frames']  is True:
        boxed_stream = OutputStream(cfg['boxed_file_name'],inp_stream.frame_shape,cfg['boxed_file_fps'],mode='save_old')
    if cfg['save_faced_frames']  is True:
        faced_stream = OutputStream(cfg['faced_file_name'],inp_stream.frame_shape,cfg['faced_file_fps'],mode='save_old')
    ...
    while user_stream.wait_key() is True:
        ...
        for pers_box in pers_boxes: pers_box.draw(outp_frame,color=cfg['obj_box_color'])
        ...
        event_type,event_fname,event_msg = clip_mgr.process_next_frame(inp_frame,outp_frame,pers_boxes)

        if cfg['event_send_message'] == True and event_type != 'nothing':
                send_message(event_type, event_fname, event_msg)
        if len(pers_boxes)>0  and cfg['save_boxed_frames'] is True: boxed_stream.write_frame(outp_frame)
        if len(face_boxes)>0 and cfg['save_faced_frames'] is True: faced_stream.write_frame(outp_frame)
        if cfg['show_output_frames'] is True:  user_stream.show('output frame',outp_frame)
        if cfg['save_output_frames'] is True:  outp_stream.write_frame(outp_frame)

    # finish:
    print('\n\n',time_meter.results())
    print(inp_stream.info())
    if cfg['save_output_frames'] is True:  print(outp_stream.info()); del outp_stream
    del inp_stream

def send_message(event_type, filename, msg):
    if   event_type == 'appeared':
        cmd_string = 'telegram-send --image \"{}\" --caption \"{}\"'.format(filename,msg)
    elif event_type == 'disappeared':
        cmd_string = 'telegram-send --file \"{}\" --caption \"{}\"'.format(filename,msg)
    else:
        print ('unknown event: {}'.format(event_type))
        return
