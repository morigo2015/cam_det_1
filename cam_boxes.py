...
class Box:
    ...
    def draw(self,frame,color=None,label=None):
        if color is None: color = BGR_GREEN
...
class ObjBox:
    ...
    def is_inside_obj_boxes(self,obj_boxes_lst):
        for obj_box in obj_boxes_lst:
            if self.is_inside_box(obj_box.box): return True
        return False

    def w_h_is_in_face_range(self):
        w = self.box.endX - self.box.startX
        h = self.box.endY - self.box.startY
        if not (cfg['face_box_width_range']   [0]        <= w <=      cfg['face_box_width_range']   [1]): return False
        if not (cfg['face_box_height_range']  [0]        <= h <=      cfg['face_box_height_range']  [1]): return False
        if not (cfg['face_box_wh_ratio_range'][0]<=float(w)/float(h)<=cfg['face_box_wh_ratio_range'][1]): return False
        return True

    def w_h_is_in_pers_range(self):
        w = self.box.endX - self.box.startX
        h = self.box.endY - self.box.startY
        if not (cfg['pers_box_width_range']   [0] <= w <=               cfg['pers_box_width_range']   [1]): return False
        if not (cfg['pers_box_height_range']  [0] <= h <=               cfg['pers_box_height_range']  [1]): return False
        if not (cfg['pers_box_wh_ratio_range'][0]<= float(w)/float(h) <=cfg['pers_box_wh_ratio_range'][1]): return False
        return True
