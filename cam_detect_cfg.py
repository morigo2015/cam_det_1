...
_cam_fps = 1.0 # actual fps processed from cam (not sent by cam)  todo calculate automatically
...
cfg = {
    ...
    'input_source': 'rtsp://admin:F112358f_bullet@192.168.1.70:554/Streaming/Channels/101',  # cam hik
    ...
    'transform_perspective' : True, # make perspective transportation to avoid lean
    ...
    'pers_filter_needed'    : True, # filter w,h range and ratio
...
#    'pers_fake_spots_filter' : False ,
    ...
    'clips_prev_frames'             : 3*int(_cam_fps) , # 5 # how many previous frames include in the begin of clip
    'clips_noperson_frames_to_stop' : 7*int(_cam_fps) , # 20 # how many frames without person are allowed to not stop clip
    'clips_frames_to_appear'        : 3*int(_cam_fps) , # 10 # how many frames in clip to allow announce 'appear' event (once per clip)
    'clips_fps'                     : 1*int(_cam_fps) ,
