...
class PersDetector:
    ...
    def detect(self, frame):
        ...
        for i in np.arange(0, detections.shape[2]):
            ...
            if confidence < cfg['pers_det_confidence']: continue
...
class FaceRecognizer:
    ...
    def __init__(self,method='knn_new'):
        ...
        assert len(self.known['encodings'])==len(self.known['labels'])==len(self.known['img_fnames'])==len(self.known['boxes'])
        self.method = method
    ...
    def recognize(self,frame,boxes):
        ...
        if self.method=='knn_old': return self.recognize_knn_old(frame,boxes)
        if self.method=='knn_new': return self.recognize_knn_new(frame,boxes)
    ...
    def recognize_knn_new(self,frame,boxes):
        ...
        for box,label,distance,index in zip(boxes,labels,distances,indexes):
            ...
            if cfg['face_rec_use_threshold']==True:
                if distance > self.threshold: label = "unknown"
    ...
    def set_threshold(self,dataset_folder=None):
        ...
        print(f'Accuracy at threshold {opt_tau:.2f} = {opt_acc:.3f}')
...
class ImageScanner:
    ...
    def __init__(self,time_meter=None, filter=True,recognizer_method='knn_new'):
    ...
    def scan(self,image):
        ...
        if self.time_meter is not None: self.time_meter.set('pers detect')
        self.pers_boxes = self.pers_detector.detect(image)  # list of boxes with persons

        if self.time_meter is not None: self.time_meter.set('face detect')
        self.facedet_boxes = self.face_detector.detect(image)

        if self.time_meter is not None: self.time_meter.set('face recognize')
        ...
        if self.time_meter is not None: self.time_meter.set('filter')
        if self.filter == True:
